...
describe('Simple NdArray', () => {
  ...
  it('Retrieves correct values for 0,0,0', async () => {
    expect(await ndarr.get(new Map().set('x', 0).set('y', 0))).toBe(0.5);
    expect(await ndarr.get(new Map().set('x', 2).set('y', 1))).toBe(null);
  });
  it('.normalizedNamedIndices: Missing indices are set to 0 and indices are set to [0,...[shape[i]]', () => {
    expect(ndarr.normalizeNamedIndices(new Map().set('x', 4).set('y', 0))).toEqual([0, 0, 0, 2]);
    expect(ndarr.normalizeNamedIndices(new Map().set('x', 3).set('y', -1))).toEqual([0, 0, 0, 2]);
  });

  it('.vLength: Is product of shape', () => {
    expect(ndarr.totalSize).toBe(ndarr.shape.reduce((l, r) => l * r));
  });
...
describe('TiledNdArray', () => {
  ...
  describe('.intersects', () => {
    it('Matches all tileSets for 0,0,1', () => {
      expect(ndarr.tileSets!.filter(ndarr.intersects([0, 0, 1])).length).toBe(3);
    });
    it('matches two tileSets for 1,5,10', () => {
      expect(ndarr.tileSets!.filter(ndarr.intersects([1, 5, 10])).length).toBe(2);
    });
    it('matches one tileSet for 2,5,10', () => {
      expect(ndarr.tileSets!.filter(ndarr.intersects([2, 5, 10])).length).toBe(1);
    });
  });
