...
describe('Reprojections', async () => {
	const cov = await Coverage.load('https://murithigeo.org/playground/coverages/grid-tiled.covjson');
...
describe('High load tests', () => {
	describe('Grid', async () => {
		...
		it('.queryData from TileSets', async () => {
			await expect(cov.queryData(['FOO'], [-100, -50])).resolves.toEqual({ FOO: 1 });
			await expect(cov.queryData(['FOO'], [100, 50])).resolves.toEqual({ FOO: 50 });
		});
	});
});
...
describe('GeoJSON Mappings', async () => {
	...
	it('the properties object is the class itself', () => {
		expect(cov.feature.properties).toBeInstanceOf(Coverage);
	});
});
