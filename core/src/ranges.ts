...
export class NdArray<T extends string | number = string | number> {
	...
	constructor(ndarr: Nd, options?: NdArrayOptions) {
		...
		this.#values = ndarray<[T | null, ...(T | null)[]]>(new Array(this.vLength), this.shape);
	...
	public get values(): [T | null, ...(T | null)[]] {
		return Array.from(this.#values.data).map((v) => (v === undefined ? null : v));
	}
	...
	async get(indices: Indices | number[]) {
		...
		const range = this.#values;
		let value = range.get(...indices);
		if (value !== undefined) return value;
		await this.loadTileset(indices);
		value = range.get(...indices); // Get directly to avoid recursion if a tile is not loaded
		return value;
	}
