...
export type PrimitiveValue = string | number | boolean | null | object | undefined; // undefined ensures that interface does not reject other members
...
export type Unit = {
	 ...
	 */
	id?: string;
} & ({ label: I18N; symbol: UnitSymbol } | { label: I18N | string } | { symbol: UnitSymbol });
...
export type ReferenceSystemObject =
	SpatialReferenceSystem | IdentifierBasedReferenceSystem | TemporalReferenceSystem;
...
export interface ValuesNdArray<T extends string | number> extends NdArrayObject {
	/**
...
export interface Coverage<D extends Domain | string = Domain | string> {
	 ...
	 */
	domainType?: D extends Domain ? D['domainType'] : DomainTypes;
	/**
 ...
 */
export interface CoverageCollection<D extends Domain | string = Domain | string> {
...
export interface MultiPoint extends DomainObject {
	...
	axes: {
		composite: { dataType: 'tuple' } & (
			| ({ coordinates: ['x', 'y']; values: Position2D[] } & WithBounds<Position2D[]>)
			| ({ coordinates: ['x', 'y', 'z']; values: Position3D[] } & WithBounds<Position3D[]>)
		);
		// eslint-disable-next-line @definitelytyped/no-single-element-tuple-type
		t?: { values: [string] } & WithBounds<[string, string]>;
	};
...
export interface Trajectory extends DomainObject {
	...
	axes: {
		...
		z?: // eslint-disable-next-line @definitelytyped/no-single-element-tuple-type
