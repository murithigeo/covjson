...
export interface Coverage<D extends Domain | string = Domain | string> {
   ...
   */
  domainType?: NonNullable<D extends Domain ? D["domainType"] : DomainTypes>;
  /**
...
export interface MultiPoint extends DomainObject {
  ...
  axes: {
    composite: { dataType: "tuple" } & (
      | ({ coordinates: ["x", "y"]; values: Position2D[] } & WithBounds<
          Position2D[]
        >)
      | ({ coordinates: ["x", "y", "z"]; values: Position3D[] } & WithBounds<
          Position3D[]
        >)
    );
    // eslint-disable-next-line @definitelytyped/no-single-element-tuple-type
