...
import type { Grid as GridDomain, Position } from "../coveragejson.d.ts";
...
export class Grid extends BaseDomain<GridDomain> {
  ...
  _reproject(referencing: Referencing): this {
    ...
    for (let i = 0; i < Math.max(xvals.length, yvals.length); i++) {
      ...
      if (x !== undefined && y !== undefined) {
        [this.axes.x.values[i], this.axes.y.values[i]] = referencing.crs([
          x,
          y,
        ]);
        continue;
      ...
      // Get the last values
      if (x === undefined) {
        this.axes.y.values[i] = referencing.crs([
    ...
    if (this.axes.z) {
      if (numAxisIsNormalized(this.axes.z)) {
        [this.axes.z.start, this.axes.z.stop] = [
          this.axes.z.start,
          this.axes.z.stop,
        ].map((v) => referencing.vrs(v));
      } else {
        this.axes.z.values = this.axes.z.values.map((v) => referencing.vrs(v));
      }
    }
    if (this.axes.t)
      this.axes.t.values = this.axes.t.values.map((v) => referencing.trs(v));
  ...
  calculateAxesBounds(timeZone?: string) {
    if ("values" in this.axes.x)
      this.axes.x.bounds = calcNumAxisBounds(this.axes.x.values);
    if ("values" in this.axes.y)
      this.axes.y.bounds = calcNumAxisBounds(this.axes.y.values);
    if (this.axes.z && "values" in this.axes.z) {
      this.axes.z.bounds = calcNumAxisBounds(this.axes.z.values);
    }
    if (this.axes.t)
      this.axes.t.bounds = calcStrAxisBounds(this.axes.t.values, timeZone);
    return this;
  }
  normalize() {
  ...
  queryIndices(point: Position): Record<"x" | "y" | "z", number> {
    const x = indexOfNearest(this.#xvals, point[0]);
    const y = indexOfNearest(this.#yvals, point[1]);
    let z = 0;
