...
import { denormalizeNumAxis } from './utils.ts';
import gbng from '../../../example-data/playground/coverages/grid-domain-bng.covjson?raw';

const grid = new Grid(JSON.parse(gbng).domain);
...
describe('Reprojection', async () => {
	const clone = (
		await new Grid({
			...
			referencing: [{ coordinates: ['x', 'y'], system: { type: 'GeographicCRS', id: 'OGC:CRS84' } }]
		}).reproject({ crsId: 'EPSG:32737' })
	).denormalize();
	it('x/y unequal no. of elements are reprojected correctly', () => {
		expect(clone.axes.x).toEqual({
			values: expect.arrayContaining([
				-4223032.968188589, -4341063.548049857, -4403892.922912131, -4406750.002024334,
				-4347530.238807505, -4478168.887842785
			])
		});
		expect(clone.axes.y).toEqual({
			values: expect.arrayContaining([
				10000000, 10721862.08474643, 11457178.300204441, 12197285.946212519, 12932369.047493774
			])
		});
	});
...
describe('BBox/Polygon Generation', () => {
	it('Expect that the length of bboxes is exactly x.values.length-1*y.values.length', () => {
		...
		const len = (grid.axes.x.num - 1) * (grid.axes.y.num - 1);
