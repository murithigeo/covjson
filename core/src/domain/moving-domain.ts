...
abstract class Base<
  T extends TrajDomain | SectionDomain,
...
export class Trajectory extends Base<TrajDomain> {
  queryIndices(point: [number, number]): Record<"composite", number> {
    const distances = this.axes.composite.values.map(([, lon, lat]) =>
      distance(point, [lon, lat]),
    );
    const [minD] = minMax(distances);
    return {
      composite:
        minD === null ? 0 : distances.findIndex((dist) => minD === dist),
    };
  }
  get z(): number[] {
    const values = this.axes.composite.values
      .map(([, , , z]) => z)
...
export class Section extends Base<SectionDomain> {
  queryIndices(point: Position): Record<"composite" | "z", number> {
    const distances = this.axes.composite.values.map(([, x, y]) =>
      distance(point, [x, y]),
    );
