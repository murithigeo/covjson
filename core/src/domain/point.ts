...
> extends BaseDomain<T> {
  ...
  _reproject(referencing: Referencing): this {
    ...
    if (this.axes.z)
      this.axes.z.values[0] = referencing.vrs(this.axes.z.values[0]);
    if (this.axes.t)
      this.axes.t.values = this.axes.t.values.map(
        referencing.trs.bind(referencing),
      );
    return this;
  }
  get z(): number[] {
