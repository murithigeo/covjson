...
import type {
  InferDomainClass,
  WithoutRegularlySpacedAxis,
} from "./domain/types.d.ts";
...
export class Coverage<T extends Domain = Domain> extends Base<CRG<T>> {
  ...
  constructor(coverage: CRG<T> & { ranges: Record<string, Nd> }) {
    ...
    this.parameterGroups =
      coverage.parameterGroups?.map((e) => new ParameterGroup(e)) || [];
    this.properties = {};
  ...
  > {
    ...
    coverage.domain.domainType =
      coverage.domain.domainType || coverage?.domainType;
  ...
  get feature(): Feature<
    ...
    { id: string; domainType: InferDomainClass<T>["domainType"] }
  > {
    return {
      type: "Feature",
  ...
  calculateIndices(
    point: [number, number],
  ): WithRequiredProperty<this, "indices"> {
  ...
  async getData(
    ...
    const data = await Promise.all(
      rangeIds.map((id) => this.ranges.get(id)?.get(indices)),
    );
